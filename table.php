<?php  
	...
	if(isset($_GET['empnum'])){
		$emp=$_GET['empnum'];
		$sql = "select emp_id,emp_num,fname,mname,lname,suffix,gender,bday,nation,civil,department,designation,empstatus from personal_info where emp_num='$emp'";
		$result = mysqli_query($con, $sql);
	}
	else{
		$sql = "select emp_id,emp_num,fname,mname,lname,suffix,gender,bday,nation,civil,department,designation,empstatus from personal_info";
		$result = mysqli_query($con, $sql);
	...
	if($_SERVER["REQUEST_METHOD"]=="POST"){
			...
			elseif(isset($_POST['ser'])){
				...
				if(empty($emps)){
					...
					header("Location: table.php?empnum=$emps");
				}
			}
	}
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$('table tr').click(function(){
					...
					window.location.href = "updateprofile.php?row_id="+id;
			...
			<table>
				...
				<tbody>
								<tr row_id="<?php echo $row['emp_id']; ?>">
									...
									<td style="border-right:none;"><?php echo $row['empstatus']; ?></td>
